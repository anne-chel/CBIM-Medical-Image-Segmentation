...
def train_net(net, args, ema_net=None, fold_idx=0):
    ...
    for epoch in range(args.epochs):
        ...
        exp_scheduler = exp_lr_scheduler_with_warmup(
            optimizer,
            ...
            if isinstance(result, tuple) or isinstance(result, list):
                for j in range(len(result)):
                    loss += args.aux_weight[j] * (
                        criterion(result[j], label.squeeze(1))
                        + criterion_dl(result[j], label)
                    )
            else:
        ...
        if not os.path.isdir(
            "%s%s/%s/" % (args.cp_path, args.dataset, args.unique_name)
        ):
        ...
        if (epoch + 1) % args.val_frequency == 0:
            ...
            dice_list_test, ASD_list_test, HD_list_test = validation(
                net_for_eval, testLoader, args
            )
            log_evaluation_result(
                writer, dice_list_test, ASD_list_test, HD_list_test, "test", epoch, args
            )
            ...
            if dice_list_test.mean() >= best_Dice.mean():
                ...
                torch.save(
                    net_for_eval.state_dict(),
                    "%s%s/%s/%d_best.pth"
                    % (args.cp_path, args.dataset, args.unique_name, fold_idx),
                )

            print("save done")
            print(
                "dice: %.5f/best dice: %.5f" % (dice_list_test.mean(), best_Dice.mean())
            )
...
def get_parser():
    ...
    parser.add_argument(
        "--dimension", type=str, default="2d", help="2d model or 3d model"
    )
    parser.add_argument(
        "--pretrain", action="store_true", help="if use pretrained weight for init"
    )

    parser.add_argument("--batch_size", default=32, type=int, help="batch size")
    parser.add_argument("--load", type=str, default=False, help="load pretrained model")
    parser.add_argument(
        "--cp_path", type=str, default="./checkpoint/", help="checkpoint path"
    )
    parser.add_argument("--log_path", type=str, default="./log/", help="log path")
    parser.add_argument(
        "--unique_name", type=str, default="test", help="unique experiment name"
    )
...
if __name__ == "__main__":
    ...
    with open("exp/exp_%s/%s.txt" % (args.dataset, args.unique_name), "w") as f:
        ...
        f.write(
            "avg Dice:"
            + str(np.mean(total_Dice, axis=0))
            + " std Dice:"
            + str(np.std(total_Dice, axis=0))
            + " mean:"
            + str(total_Dice.mean())
            + " std:"
            + str(np.mean(total_Dice, axis=1).std())
            + "\n"
        )
        f.write(
            "avg HD:"
            + str(np.mean(total_HD, axis=0))
            + " std HD:"
            + str(np.std(total_HD, axis=0))
            + " mean:"
            + str(total_HD.mean())
            + " std:"
            + str(np.mean(total_HD, axis=1).std())
            + "\n"
        )
        f.write(
            "avg ASD:"
            + str(np.mean(total_ASD, axis=0))
            + " std ASD:"
            + str(np.std(total_ASD, axis=0))
            + " mean:"
            + str(total_ASD.mean())
            + " std:"
            + str(np.mean(total_ASD, axis=1).std())
            + "\n"
        )
