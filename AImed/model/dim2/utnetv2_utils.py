...
class BidirectionAttention(nn.Module):
    def __init__(
        ...
        if proj_type == "linear":
            self.feat_qv = nn.Conv2d(
                feat_dim, self.inner_dim * 2, kernel_size=1, bias=False
            )
            self.feat_out = nn.Conv2d(
                self.inner_dim, out_dim, kernel_size=1, bias=False
            )
...
class SemanticMapFusion(nn.Module):
    def __init__(self, in_dim_list, dim, heads, depth=1, norm=nn.BatchNorm2d):
        ...
        if depth == 0:
            ...
            for i in range(len(in_dim_list)):
                self.in_proj.append(
                    nn.Conv2d(in_dim_list[i], dim, kernel_size=1, bias=False)
                )

            self.fusion = TransformerBlock(
                dim, depth, heads, dim // heads, dim, attn_drop=0.0, proj_drop=0.0
            )
            ...
            for i in range(len(in_dim_list)):
                self.out_proj.append(
                    nn.Conv2d(dim, in_dim_list[i], kernel_size=1, bias=False)
                )
    ...
    def forward(self, map_list):
        ...
        if self.depth == 0:
            ...
            maps_out = [
                self.out_proj[i](
                    attned_maps[i].permute(0, 2, 1).view(B, self.dim, H, W)
                )
                for i in range(len(map_list))
            ]
...
class inconv(nn.Module):
    def __init__(
        self, in_ch, out_ch, block=BasicBlock, norm=nn.BatchNorm2d, act=nn.GELU
    ):
        super().__init__()
...
class up_block(nn.Module):
    def __init__(
        ...
        self.reduction = nn.Conv2d(
            in_ch + out_ch, out_ch, kernel_size=1, padding=0, bias=False
        )
