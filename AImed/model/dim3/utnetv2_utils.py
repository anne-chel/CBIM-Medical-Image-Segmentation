...
class BidirectionAttention(nn.Module):
    def __init__(
        ...
        if proj_type == "linear":
            ...
            self.feat_out = nn.Conv3d(
                self.inner_dim, out_dim, kernel_size=1, stride=1, padding=0, bias=False
            )
        else:
            self.feat_qv = DepthwiseSeparableConv(
                feat_dim, self.inner_dim * 2, kernel_size=kernel_size
            )
            self.feat_out = DepthwiseSeparableConv(
                self.inner_dim, out_dim, kernel_size=kernel_size
            )

        self.map_qv = nn.Conv3d(
            map_dim, self.inner_dim * 2, kernel_size=1, stride=1, padding=0, bias=False
        )
        self.map_out = nn.Conv3d(
            self.inner_dim, map_dim, kernel_size=1, stride=1, padding=0, bias=False
        )
    ...
    def forward(self, feat, semantic_map):
        ...
        map_q, map_v = self.map_qv(semantic_map).chunk(
            2, dim=1
        )  # B, inner_dim, ms, ms, ms
        ...
        feat_map_attn = F.softmax(
            attn, dim=-1
        )  # semantic map is very concise that don't need dropout,
...
class BidirectionAttentionBlock(nn.Module):
    def __init__(
        ...
        self.norm1 = (
            norm(feat_dim) if norm else nn.Identity()
        )  # norm layer for feature map
        self.norm2 = (
            norm(map_dim) if norm else nn.Identity()
        )  # norm layer for semantic map

        self.attn = BidirectionAttention(
            feat_dim,
            map_dim,
            out_dim,
            heads,
        ...
        if feat_dim != out_dim:
            self.shortcut = ConvNormAct(
                feat_dim, out_dim, 1, padding=0, norm=norm, act=act, preact=True
            )

        if proj_type == "linear":
            self.feedforward = FusedMBConv(
                out_dim, out_dim, expansion=expansion, kernel_size=1, act=act, norm=norm
            )
        else:
            self.feedforward = MBConv(
                out_dim,
                out_dim,
                expansion=expansion,
                kernel_size=kernel_size,
...
class PatchMerging(nn.Module):
    ...
    def __init__(
        ...
        if proj_type == "linear":
            ...
            self.reduction = DepthwiseSeparableConv(
                merged_dim, out_dim, kernel_size=kernel_size
            )
...
class SemanticMapGeneration(nn.Module):
    def __init__(self, feat_dim, map_dim, map_size):
        ...
        self.base_proj = nn.Conv3d(
            feat_dim, map_dim, kernel_size=3, padding=1, bias=False
        )

        self.semantic_proj = nn.Conv3d(
            feat_dim, self.map_code_num, kernel_size=3, padding=1, bias=False
        )
    ...
    def forward(self, x):
        ...
        return semantic_map.view(
            B, self.map_dim, self.map_size[0], self.map_size[1], self.map_size[2]
        )
    ...
    def __init__(
        ...
        for i in range(len(in_dim_list)):
            self.out_proj.append(
                nn.Conv3d(dim, in_dim_list[i], kernel_size=1, bias=False)
            )

    def forward(self, map_list):
        B, _, D, H, W = map_list[0].shape
        proj_maps = [
            self.in_proj[i](map_list[i]).view(B, self.dim, -1).permute(0, 2, 1)
        ...
        maps_out = [
            self.out_proj[i](attned_maps[i].permute(0, 2, 1).view(B, self.dim, D, H, W))
            for i in range(len(map_list))
        ]
...
class inconv(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv3d(
            in_ch, out_ch, kernel_size=kernel_size, padding=pad_size, bias=False
        )
...
class down_block(nn.Module):
    def __init__(
        ...
        for i in range(conv_num):
            block_list.append(
                conv_block(out_ch, out_ch, norm=norm, act=act, kernel_size=kernel_size)
            )
        self.conv_blocks = nn.Sequential(*block_list)

        self.trans_blocks = BasicLayer(
            out_ch,
            map_dim,
            out_ch,
            num_blocks=trans_num,
            heads=heads,
            dim_head=dim_head,
            norm=norm,
            act=act,
...
class up_block(nn.Module):
    def __init__(
        ...
        map_dim = out_ch if map_dim is None else map_dim
        if map_shortcut:
            self.map_reduction = nn.Conv3d(
                in_ch + out_ch, map_dim, kernel_size=1, bias=False
            )
        else:
        ...
        for i in range(conv_num):
            conv_list.append(
                conv_block(dim1, out_ch, kernel_size=kernel_size, norm=norm, act=act)
            )
            dim1 = out_ch
