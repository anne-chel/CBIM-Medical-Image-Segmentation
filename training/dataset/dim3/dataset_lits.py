...
class LiverDataset(Dataset):
    def __init__(self, args, mode="train", k_fold=5, k=0, seed=0):
        ...
        test_name_list = img_name_list[
            k * (length // k_fold) : (k + 1) * (length // k_fold)
        ]
        ...
        for name in img_name_list:
            ...
            self.spacing_list.append(
                spacing[::-1]
            )  # itk axis order is inverse of numpy axis order
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            ...
            if np.random.random() < 0.15:
                tensor_img = augmentation.brightness_multiply(
                    tensor_img, multiply_range=[0.7, 1.3]
                )
            if np.random.random() < 0.15:
                tensor_img = augmentation.gamma(tensor_img, gamma_range=[0.7, 1.5])
            if np.random.random() < 0.15:
                tensor_img = augmentation.contrast(
                    tensor_img, contrast_range=[0.65, 1.5]
                )
            ...
            if np.random.random() < 0.2:
                ...
                tensor_img, tensor_lab = augmentation.crop_3d(
                    tensor_img, tensor_lab, [d + 70, h + 70, w + 70], mode="random"
                )
