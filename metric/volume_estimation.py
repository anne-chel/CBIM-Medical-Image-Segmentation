...
def calculate_volume(patient=10, mask_2chamber=None, mask_4chamber=None):
    # print('Initilised ventricle widths', ventricle_widths)
    for mode in ["ED", "ES"]:
        for ch in [2, 4]:
            if patient < 10:
                filename = (
                    f"../data/raw/training/patient000{patient}/patient000{patient}_{ch}CH_"
                    + mode
                    + "_gt.mhd"
                )
            elif patient < 100:
                filename = (
                    f"../data/raw/training/patient00{patient}/patient00{patient}_{ch}CH_"
                    + mode
                    + "_gt.mhd"
                )
            elif patient < 1000:
                filename = (
                    f"../data/raw/training/patient0{patient}/patient0{patient}_{ch}CH_"
                    + mode
                    + "_gt.mhd"
                )
            ...
            if ch == 2:
                mask_2chamber = np.copy(label_npa_zslice)
            else:
                mask_4chamber = np.copy(label_npa_zslice)
    ...
    if patient < 10:
        filename = f"../data/raw/training/patient000{patient}/Info_{ch}CH.cfg"
    elif patient < 100:
        filename = f"../data/raw/training/patient00{patient}/Info_{ch}CH.cfg"
    elif patient < 1000:
        filename = f"../data/raw/training/patient0{patient}/Info_{ch}CH.cfg"
    with open(filename) as info:
        ...
        true_ef = info["LVef"]
    print(
        f"Patient {patient}, true EF: {true_ef:.3},estimated EF: {ejection_fraction:.3}"
    )
    # compute the error
