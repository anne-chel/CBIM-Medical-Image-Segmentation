...
class UTNetV2(nn.Module):
    def __init__(
        ...
        self.map_fusion = SemanticMapFusion(
            chan_num[1:4], fusion_dim, fusion_heads, depth=fusion_depth, norm=norm
        )
    ...
    def forward(self, x):
        ...
        if self.aux_loss:
            ...
            aux_out = F.interpolate(
                aux_out, size=x.shape[-3:], mode="trilinear", align_corners=True
            )
